        ...
        public static function sendUserMessage() 
		{
            ...
            if ($is_cometchat) {
                ...
                if ($api_msg_response && $api_msg_response->success->status == 2000) {
                    ...
                    $json['message'] 	= esc_html__('Message has been sent','workreap_core');
                } else {
                    $json['type'] = 'error';
                    $json['msg_type'] = 'normal_comet';
                    $json['message'] = !empty($api_msg_response->failed->message) ? $api_msg_response->failed->message : esc_html__('Something went wrong with cometchat API.','workreap_core');
                }
            } else if ($is_wpguppy) {
